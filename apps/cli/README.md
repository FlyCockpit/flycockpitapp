...
The ordinary CLI installers and Homebrew formula are generated by cargo-dist
and published with each GitHub Release; see `dist-workspace.toml` and the
`Release CLI` workflow. Privileged Linux deployment uses a separate,
target-specific containment bundle and checksum from the same release.
...
Installation requires systemd 253 or newer for `OpenFile=`, stages replacements
with atomic renames, validates the installed unit set before starting it, stops
