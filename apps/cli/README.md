...
The shell installers are generated by cargo-dist and published with each
GitHub Release. The owner-maintained Homebrew formula builds the tagged source
with the `no-self-update` feature; the release workflow verifies that contract
without editing the separate tap repository.
...
| `cockpit update [--check] [--status]` (`cockpit self-update`) | Check or apply receipt-gated installed-binary updates. Homebrew installations are directed to `brew upgrade cockpit`; other package-manager installs are refused. |
...
Cockpit has no telemetry, analytics, or crash-reporting service. Its self-update
client runs only for cargo-dist receipt installations and fails closed when the
production TUF trust root is absent. The User-Agent identifies Cockpit version, operating system, and
