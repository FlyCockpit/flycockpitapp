...
- CrofAI (`crofai`): OpenAI Chat Completions and Responses at `https://crof.ai/v1`; defaults to `Authorization: Bearer $CROF_API_KEY` and also recognizes `CROFAI_API_KEY`. Models are fetched from `/v1/models`; the template leaves wire selection on `auto` so model routing can choose Chat Completions or Responses. CrofAI's Anthropic host (`https://anthropic.nahcrof.com`) is documented as a setup hint, not a separate provider template.
