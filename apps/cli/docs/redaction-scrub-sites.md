...
- Client boundary: `crates/cockpit-core/src/daemon/server/dispatch.rs`, `crates/cockpit-core/src/daemon/server/mod.rs`, `crates/cockpit-core/src/daemon/mod.rs`, `crates/cockpit-core/src/daemon/fs_api.rs`, `crates/cockpit-core/src/redact/coverage_authority.rs`
...
- `crates/cockpit-core/src/tools/mcp_tool.rs`: MCP tool responses are scrubbed before they enter model-visible tool output.
...
- `crates/cockpit-core/src/daemon/mod.rs`: `EventScrub` is the scrub-only coverage an event envelope carries. A session-originated event on the daemon-global bus (an LSP notice, a session's host-capability publication) is scrubbed with the current daemon-global table and the originating session's table matched together in one pass (an unbound, scrub-only merge that can never be published or bound), so a secret overlapping both tables leaves no suffix. When coverage cannot be established (or, for a host-capability receipt recovered on behalf of another session, the originating session's coverage is not available), owners receive the event and every other principal receives only its content-free projection, or nothing. The global-coverage module (daemon/global_coverage.rs, which scrubs nothing itself) is the one delivery funnel for the daemon-global bus: it republishes a stale global table before scrubbing, and publishes each table with its freshness stamps atomically and monotonically.
...
- `crates/cockpit-core/src/approval/policy.rs`: approval policy text is scrubbed through the redaction table before it is rendered, so policy explanations never echo secret-bearing command output.
- `crates/cockpit-core/src/conversation_rules.rs`: conversation rule text is scrubbed before persisted rules are rendered into model-visible context.
- `crates/cockpit-core/src/skills/mod.rs`: skill rendering scrubbers (command output at the substitution site, known-secret error markers) run rendered skill bodies through the redaction table before they reach model context.
- `crates/cockpit-core/src/mcp/network.rs`: MCP network-facing values are scrubbed before they leave the network boundary.
- `crates/cockpit-core/src/mcp/sandbox.rs`: sandboxed MCP tool results, errors, and Monty objects (strings, paths, type names) are scrubbed before crossing back into Cockpit.
