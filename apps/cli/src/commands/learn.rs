//! Shared `/learn` and `cockpit assistants learn` request construction.
//!
...
mod tests {
    ...
    fn learn_slash_and_cli_compose_same() {
        let cli = Cli::try_parse_from(["cockpit", "assistants", "learn", "how", "we", "deployed"])
            .unwrap();
        let Some(Command::Assistants(AssistantCommand::Learn(args))) = cli.command else {
            panic!("expected assistants learn command");
        };
