...
- `task(intent, delegate|batch|control)` — delegate when a separate context is cleaner. Use `{"intent":"delegate","delegate":{"agent":"builder","prompt":"..."}}` for larger or isolated implementation, `{"intent":"delegate","delegate":{"agent":"explore","prompt":"..."}}` for broad read-only investigation, and `{"intent":"delegate","delegate":{"agent":"docs","prompt":"{\"package\":\"<name>\",\"question\":\"<usage question>\"}"}}` for dependency API usage sourced from the dependency's real code.
