...
- `task(intent, delegate|batch|control)` — use `docs` by default when a third-party dependency API is unfamiliar, version-sensitive, signature-sensitive, or not clearly established by already-read local code. Use `{"intent":"delegate","delegate":{"agent":"docs","prompt":"{\"package\":\"<name>\",\"question\":\"<usage question>\"}"}}`; get back a `file:line`-cited answer from the dependency's real code. Spend those tokens for correctness rather than guess or web-search; use a web tool only for what `docs` can't answer.
