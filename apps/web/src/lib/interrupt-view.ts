import type {
  CommandDetail,
  GrantKind,
  InterruptOption,
  InterruptQuestion,
  ResolveResponse,
  SandboxEscalation,
} from "@flycockpit/cockpit-protocol";

export type RiskVariant = "neutral" | "low" | "medium" | "high" | "critical";

export type InterruptSelection =
  | { kind: "single"; selectedId: string }
  | { kind: "multi"; selectedIds: string[] }
  | { kind: "freetext"; text: string }
  | { kind: "cancel" };

export type ScopeChoiceView = {
  scope: string;
  optionId: string;
  labelKey: string;
};

export type CommandDetailView = {
  fullCommand: string;
  cwd?: string;
  stepLabel: string;
  risk: { tier: string | null; label: string; variant: RiskVariant };
  reasons: string[];
  affectedTargets: string[];
  nativeToolHints: string[];
  offeredScopes: string[];
  scopeChoices: ScopeChoiceView[];
  policyCap?: string;
  writeContent?: { preview: string; truncated: boolean; dynamic: boolean };
};

export type SandboxEscalationView = {
  confinedExit: number;
  confinedStderrPreview: string;
  confinedStderrTruncated: boolean;
  suggestedPaths: string[];
  suggestedAccess?: string;
  denial?: { confidence: string; evidenceCount: number };
};

export type InterruptView =
  | {
      kind: "single";
      prompt: string;
      readOnly: boolean;
      canResolve: boolean;
      permission: boolean;
      frame: "approval" | "question";
      freeText: boolean;
      primaryOptions: InterruptOption[];
      secondaryOptions: InterruptOption[];
      commandDetail?: CommandDetailView;
      approvalClass?: GrantKind;
      approvalClassLabelKey?: string;
      sandboxEscalation?: SandboxEscalationView;
      escalationOptionIds: string[];
    }
  | {
      kind: "multi";
      prompt: string;
      readOnly: boolean;
      canResolve: boolean;
      frame: "question";
      freeText: boolean;
      primaryOptions: InterruptOption[];
      secondaryOptions: InterruptOption[];
    }
  | {
      kind: "freetext";
      prompt: string;
      readOnly: boolean;
      canResolve: boolean;
      frame: "question";
      masked: boolean;
      inputType: "password" | "text";
    };

const WRITE_CONTENT_PREVIEW_LIMIT = 600;
const STDERR_PREVIEW_LIMIT = 800;

export function interruptView(
  question: InterruptQuestion,
  options: { readOnly?: boolean } = {},
): InterruptView {
  const readOnly = options.readOnly === true;
  const canResolve = !readOnly;

  if (question.kind === "freetext") {
    const masked = question.data.masked === true;
    return {
      kind: "freetext",
      prompt: question.data.prompt,
      readOnly,
      canResolve,
      frame: "question",
      masked,
      inputType: masked ? "password" : "text",
    };
  }

  const optionSplit = splitOptions(question.data.options);

  if (question.kind === "multi") {
    return {
      kind: "multi",
      prompt: question.data.prompt,
      readOnly,
      canResolve,
      frame: "question",
      freeText: question.data.allow_freetext === true,
      primaryOptions: optionSplit.primaryOptions,
      secondaryOptions: optionSplit.secondaryOptions,
    };
  }

  const permission = question.data.permission === true;
  const sandboxEscalation = question.data.sandbox_escalation
    ? sandboxEscalationView(question.data.sandbox_escalation)
    : undefined;
  const scopeChoices = scopeChoicesFromOptions(
    question.data.command_detail?.offered_scopes ?? [],
    question.data.options,
    sandboxEscalation !== undefined,
  );
  const scopeOptionIds = new Set(scopeChoices.map((scope) => scope.optionId));
  const singleOptions = splitOptions(
    question.data.options.filter((option) => !scopeOptionIds.has(option.id)),
    sandboxEscalation !== undefined,
  );

  return {
    kind: "single",
    prompt: question.data.prompt,
    readOnly,
    canResolve,
    permission,
    frame: permission ? "approval" : "question",
    freeText: !permission && question.data.allow_freetext === true,
    primaryOptions: singleOptions.primaryOptions,
    secondaryOptions: singleOptions.secondaryOptions,
    commandDetail: question.data.command_detail
      ? commandDetailView(question.data.command_detail, scopeChoices)
      : undefined,
    approvalClass: question.data.approval_class,
    approvalClassLabelKey: question.data.approval_class
      ? approvalClassLabelKey(question.data.approval_class)
      : undefined,
    sandboxEscalation,
    escalationOptionIds: sandboxEscalation
      ? singleOptions.secondaryOptions
          .filter((option) => option.id.startsWith("escalate_"))
          .map((option) => option.id)
      : [],
  };
}

export function resolveFromSelection(
  question: InterruptQuestion,
  selection: InterruptSelection,
): ResolveResponse {
  switch (selection.kind) {
    case "cancel":
      return { kind: "cancel" };
    case "single":
      if (question.kind !== "single") {
        throw new Error(`Cannot resolve ${question.kind} interrupt with single selection.`);
      }
      return { kind: "single", data: { selected_id: selection.selectedId } };
    case "multi":
      if (question.kind !== "multi") {
        throw new Error(`Cannot resolve ${question.kind} interrupt with multi selection.`);
      }
      return { kind: "multi", data: { selected_ids: selection.selectedIds } };
    case "freetext":
      if (question.kind === "single" && question.data.permission === true) {
        throw new Error("Permission interrupts do not accept freetext responses.");
      }
      if (question.kind !== "freetext" && question.data.allow_freetext !== true) {
        throw new Error(`Cannot resolve ${question.kind} interrupt with freetext.`);
      }
      return { kind: "freetext", data: { text: selection.text } };
  }
}

export function maskedDisplayValue(text: string, masked: boolean) {
  return masked ? "*".repeat(text.length) : text;
}

export function riskVariant(tier: string | undefined): RiskVariant {
  const normalized = tier?.trim().toLowerCase() ?? "";
  if (normalized === "ordinary" || normalized === "low") return "low";
  if (normalized === "mutating" || normalized === "medium") return "medium";
  if (normalized === "destructive" || normalized === "high") return "high";
  if (normalized === "privileged" || normalized === "critical") return "critical";
  return "neutral";
}

function splitOptions(options: InterruptOption[], sandboxEscalation = false) {
  const isSecondary = (option: InterruptOption) =>
    option.secondary === true || (sandboxEscalation && option.id.startsWith("escalate_"));
  return {
    primaryOptions: options.filter((option) => !isSecondary(option)),
    secondaryOptions: options.filter(isSecondary),
  };
}

function commandDetailView(
  detail: CommandDetail,
  scopeChoices: ScopeChoiceView[],
): CommandDetailView {
  const writeContent = detail.write_content
    ? truncatePreview(detail.write_content.content, WRITE_CONTENT_PREVIEW_LIMIT)
    : undefined;
  return {
    fullCommand: detail.full_command,
    cwd: detail.cwd,
    stepLabel: `Step ${detail.step} of ${detail.step_count}`,
    risk: {
      tier: detail.risk_tier ?? null,
      label: detail.risk_tier ?? "unknown",
      variant: riskVariant(detail.risk_tier),
    },
    reasons: detail.risk_reasons ?? [],
    affectedTargets: detail.affected_targets ?? [],
    nativeToolHints: detail.native_tool_hints ?? [],
    offeredScopes: detail.offered_scopes ?? [],
    scopeChoices,
    policyCap: detail.policy_cap,
    writeContent: writeContent
      ? {
          preview: writeContent.preview,
          truncated: writeContent.truncated,
          dynamic: detail.write_content?.dynamic === true,
        }
      : undefined,
  };
}

function sandboxEscalationView(escalation: SandboxEscalation): SandboxEscalationView {
  const stderr = truncatePreview(escalation.confined_stderr, STDERR_PREVIEW_LIMIT);
  return {
    confinedExit: escalation.confined_exit,
    confinedStderrPreview: stderr.preview,
    confinedStderrTruncated: stderr.truncated,
    suggestedPaths: escalation.suggested_paths ?? [],
    suggestedAccess: escalation.suggested_access,
    denial: escalation.denial
      ? {
          confidence: escalation.denial.confidence,
          evidenceCount: escalation.denial.evidence.length,
        }
      : undefined,
  };
}

function scopeChoicesFromOptions(
  offeredScopes: string[],
  options: InterruptOption[],
  sandboxEscalation: boolean,
) {
  const optionsById = new Map(options.map((option) => [option.id, option]));
  return offeredScopes.flatMap((scope) => {
    const optionId = sandboxEscalation ? `escalate_grant_${scope}` : `approve_${scope}`;
    return optionsById.has(optionId) ? [{ scope, optionId, labelKey: scopeLabelKey(scope) }] : [];
  });
}

function approvalClassLabelKey(kind: GrantKind) {
  if (kind === "mcp_tool") return "remote.interruptClassMcpTool";
  if (kind === "path") return "remote.interruptClassPath";
  return "remote.interruptClassCommand";
}

function scopeLabelKey(scope: string) {
  if (scope === "once") return "remote.interruptScopeOnce";
  if (scope === "session") return "remote.interruptScopeSession";
  if (scope === "project") return "remote.interruptScopeProject";
  if (scope === "global") return "remote.interruptScopeGlobal";
  return "remote.interruptScopeUnknown";
}

function truncatePreview(value: string, limit: number) {
  if (value.length <= limit) return { preview: value, truncated: false };
  return { preview: `${value.slice(0, limit)}...`, truncated: true };
}
