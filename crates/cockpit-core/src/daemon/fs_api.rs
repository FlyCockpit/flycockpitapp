...
pub(crate) fn fs_rename_precondition_digest(
    project_root: &str,
    from_path: &str,
) -> Result<Option<[u8; 32]>, ErrorPayload> {
    let root = canonical_project_root(project_root)?;
    let source = resolve_rename_source(&root, from_path)?;
    ...
    if metadata.is_file() {
        digest.update(std::fs::read(&source).map_err(internal)?);
...
fn rename_sidecar(root: &Path, to_path: &str, artifact_id: &str) -> Result<PathBuf, ErrorPayload> {
    let target = resolve_for_write(root, to_path)?;
    let parent = target
        .parent()
        .ok_or_else(|| bad_request("rename target has no parent"))?;
...
pub(crate) fn stage_remote_rename_sync(
    ...
    let parent = sidecar
        .parent()
        .ok_or_else(|| bad_request("rename sidecar has no parent"))?;
    std::fs::create_dir_all(parent).map_err(internal)?;
    let mut expected = artifact_id.as_bytes().to_vec();
    expected.extend_from_slice(&precondition_digest);
    expected.extend_from_slice(&result_digest);
    if let Ok(existing) = std::fs::read(&sidecar) {
        if existing != expected {
            return Err(bad_request(
                "remote rename sidecar conflicts with prepared evidence",
            ));
        }
        return Ok(());
    }
    let mut file = std::fs::OpenOptions::new()
        ...
        .open(&sidecar)
        .map_err(internal)?;
    file.write_all(&expected).map_err(internal)?;
    file.sync_all().map_err(internal)?;
    std::fs::File::open(parent)
        .and_then(|dir| dir.sync_all())
        .map_err(internal)
...
pub(crate) fn apply_or_reconcile_remote_rename_sync(
    ctx: &DaemonContext,
    project_root: &str,
    from_path: &str,
    to_path: &str,
    ...
    precondition_digest: [u8; 32],
    result_digest: [u8; 32],
) -> Result<(Response, PathBuf), ErrorPayload> {
    let root = canonical_project_root(project_root)?;
    let from = resolve_rename_source(&root, from_path)?;
    let to = resolve_for_write(&root, to_path)?;
    let sidecar = rename_sidecar(&root, to_path, artifact_id)?;
    let mut expected = artifact_id.as_bytes().to_vec();
    expected.extend_from_slice(&precondition_digest);
    expected.extend_from_slice(&result_digest);
    if std::fs::read(&sidecar).map_err(internal)? != expected {
        return Err(bad_request(
            "remote rename evidence does not match the operation",
        ));
    }
    let locks = ctx.registry.locks();
    let _from_guard = locks
    ...
    let _to_guard = locks
        .acquire_transient(&to, REMOTE_FILE_AGENT)
        .map_err(lock_conflict)?;
    if from.try_exists().map_err(internal)? {
        if fs_rename_precondition_digest(project_root, from_path)? != Some(precondition_digest) {
            return Err(ErrorPayload {
                code: ErrorCode::HashMismatch,
                message: "rename source changed after preparation".into(),
            });
        }
        std::fs::rename(&from, &to).map_err(internal)?;
    } else if !to.try_exists().map_err(internal)? {
        return Err(bad_request(
            "rename has neither source nor operation-owned result",
        ));
    }
...
mod tests {
    ...
    #[test]
    fn staged_rename_reconciles_crash_after_atomic_rename() {
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path().join("app");
        std::fs::create_dir_all(root.join("nested")).unwrap();
        std::fs::write(root.join("nested/from.txt"), b"rename me").unwrap();
        let ctx = test_ctx(&root);
        let root_text = root.to_str().unwrap();
        let artifact = "00000000-0000-4000-8000-0000000000a2";
        let pre = fs_rename_precondition_digest(root_text, "nested/from.txt")
            .unwrap()
            .unwrap();
        let result = [42; 32];
        stage_remote_rename_sync(root_text, "nested/to.txt", artifact, pre, result).unwrap();
        let (_, sidecar) = apply_or_reconcile_remote_rename_sync(
            &ctx,
            root_text,
            "nested/from.txt",
            "nested/to.txt",
        ...
        .unwrap();
        assert!(!root.join("nested/from.txt").exists());
        assert_eq!(
            std::fs::read(root.join("nested/to.txt")).unwrap(),
            b"rename me"
        );
        assert!(
            sidecar.exists(),
            "operation evidence survives the crash cut"
        );
        apply_or_reconcile_remote_rename_sync(
            &ctx,
            root_text,
            "nested/from.txt",
            "nested/to.txt",
