...
fn write_worker_ready_report(close: bool) -> Result<()> {
    ...
    if close && WORKER_READINESS_CHANNEL_CLOSED.swap(true, Ordering::AcqRel) {
        bail!("worker readiness channel is already closed");
    }
    #[cfg(unix)]
    {
...
/// A worker's fatal error before its readiness barrier, reported on the same
/// channel readiness would have used (the fd-4 pipe on Unix, a staged file
/// beside the staged pipe identity on Windows). It carries the worker's own
/// error chain — the diagnostic the person starting the daemon needs, such as
/// an incompatible database schema — so the supervisor's error, and through
/// it `daemon start`, says why the worker failed rather than only that it
/// exited. The chain is the one the worker already logs to `daemon.log`;
/// error chains never carry secret material.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
struct WorkerBootFailureReport {
    protocol_version: u32,
    pid: u32,
    generation: u64,
    boot_failure: String,
...
/// One frame on the worker readiness channel. Untagged so a ready frame keeps
/// its exact wire shape: a boot failure never carries `opened_at_unix_ms`,
/// and a ready frame never carries `boot_failure`.
#[cfg(any(unix, test))]
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
#[serde(untagged)]
enum WorkerReadinessLine {
    Ready(WorkerReadyReport),
    BootFailed(WorkerBootFailureReport),
}

/// Upper bound on a reported boot-failure chain, well under the readiness
/// frame limit.
const MAX_BOOT_FAILURE_BYTES: usize = 16 * 1024;

/// A supervised worker reported a fatal error before readiness. Its display
/// is the worker's own error chain, prefixed with the worker's identity.
#[derive(Debug)]
pub(crate) struct WorkerBootFailed {
    pid: u32,
    generation: u64,
    reason: String,
}

impl std::fmt::Display for WorkerBootFailed {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            formatter,
            "worker pid {} (generation {}) failed before readiness: {}",
            self.pid, self.generation, self.reason
        )
    }
}

impl std::error::Error for WorkerBootFailed {}

/// Whether this worker's readiness channel is still open for a report. It
/// closes at the (final) readiness report or at a boot-failure report, so a
/// failure after readiness is never mistaken for a boot failure.
static WORKER_READINESS_CHANNEL_CLOSED: AtomicBool = AtomicBool::new(false);

fn boot_failure_text(error: &anyhow::Error) -> String {
    let mut text = super::daemon_log::error_text_for_daemon_log(error);
    if text.len() > MAX_BOOT_FAILURE_BYTES {
        let mut end = MAX_BOOT_FAILURE_BYTES;
...
/// Report a supervised worker's fatal pre-readiness error to its supervisor.
///
/// Idempotent and a no-op outside a supervised worker or once readiness has
/// been reported: callers on every worker exit-with-error path (the daemon
/// boot wrapper, the CLI entry point) may all call it; the first report wins.
pub fn report_worker_boot_failure(error: &anyhow::Error) {
    if !is_worker_process() || WORKER_READINESS_CHANNEL_CLOSED.swap(true, Ordering::AcqRel) {
        return;
    }
    let report = WorkerBootFailureReport {
        protocol_version: super::proto::PROTOCOL_VERSION,
        pid: std::process::id(),
        generation: worker_generation(),
        boot_failure: boot_failure_text(error),
    };
    ...
    #[cfg(unix)]
    {
        ...
        encoded.push(b'\n');
        // SAFETY: fd 4 is the inherited write end of this generation's
        // readiness pipe (close-on-exec since entry); `encoded` stays valid
        // for the write. The descriptor is blocking and the frame is bounded
        // well below the pipe capacity, so one write delivers it whole.
        let written = unsafe { libc::write(READY_FD, encoded.as_ptr().cast(), encoded.len()) };
        if written != isize::try_from(encoded.len()).unwrap_or(-1) {
            super::daemon_log::daemon_eprintln!(
                "supervised worker pid {}: could not report boot failure to the supervisor: {}",
                std::process::id(),
    ...
    #[cfg(windows)]
    if let Some(identity) = std::env::var_os(WINDOWS_IDENTITY_ENV) {
        let path = windows_boot_failure_path(Path::new(&identity));
        if let Err(error) = std::fs::write(&path, &encoded) {
            super::daemon_log::daemon_eprintln!(
                "supervised worker pid {}: could not report boot failure to the supervisor: {error}",
                std::process::id()
            );
        }
    }
...
/// Where a Windows worker stages its boot-failure report: beside the staged
/// pipe identity the supervisor already waits on, in the private state dir.
#[cfg(windows)]
fn windows_boot_failure_path(staged_identity: &Path) -> PathBuf {
    let mut path = staged_identity.as_os_str().to_owned();
    path.push(".boot-failure");
    PathBuf::from(path)
}

/// Consume a Windows worker's staged boot-failure report, if it wrote one
/// for exactly this process and generation.
#[cfg(windows)]
fn take_windows_boot_failure(
    staged_identity: &Path,
    pid: u32,
    generation: u64,
) -> Option<WorkerBootFailed> {
    let path = windows_boot_failure_path(staged_identity);
    let encoded = std::fs::read(&path).ok()?;
    let _ = std::fs::remove_file(&path);
    if encoded.len() > MAX_ADMIN_LINE {
        return None;
    }
    let report: WorkerBootFailureReport = serde_json::from_slice(&encoded).ok()?;
    (report.protocol_version == super::proto::PROTOCOL_VERSION
        && report.pid == pid
        && report.generation == generation)
        .then_some(WorkerBootFailed {
            pid,
            generation,
            reason: report.boot_failure,
        })
}
...
/// Run the supervisor until it stops. A failure before the first worker is
/// ready — the supervisor's own, or a worker's typed boot failure — is also
/// reported to a detached launcher waiting on the spawn channel, so
/// `daemon start` shows the same diagnostic as `daemon start --foreground`.
#[cfg(any(unix, windows))]
pub async fn run(paths: DaemonPaths, no_sandbox: bool, resume_all_sessions: bool) -> Result<()> {
    let result = run_supervisor(paths, no_sandbox, resume_all_sessions).await;
    if let Err(error) = &result {
        // A no-op once readiness was reported (one report per launch).
        super::spawn_notify::report_err(error);
    }
    result
}
...
fn spawn_ready_worker_blocking(request: OwnedWorkerSpawnRequest) -> Result<Worker> {
    ...
    command
        ...
        // The supervisor is the one spawn reporter: a worker reports its
        // readiness or boot failure only to the supervisor, which reports the
        // outcome (the worker's typed failure included) to its launcher.
        .env_remove(super::DAEMON_SPAWN_NOTIFY_ENV)
        .stdin(Stdio::null())
    ...
    #[cfg(windows)]
    {
        ...
        remove_if_present(&windows_boot_failure_path(&staged_identity))?;
    ...
    #[cfg(windows)]
    {
        if !wait_windows_identity(&staged_identity, &mut child, super::DAEMON_SPAWN_TIMEOUT)? {
            ...
            // Read after the reap: a worker writes its report before exit.
            if let Some(failure) =
                take_windows_boot_failure(&staged_identity, pid, request.generation)
            {
        ...
        let _ = remove_if_present(&windows_boot_failure_path(&staged_identity));
...
/// What one nonblocking drain of the readiness pipe produced.
#[cfg(unix)]
enum ReadinessPipeRead {
    /// A complete newline-terminated frame is buffered.
    Frame(usize),
...
/// Read everything currently available on the nonblocking readiness pipe.
#[cfg(unix)]
fn drain_readiness_pipe(fd: libc::c_int, encoded: &mut Vec<u8>) -> Result<ReadinessPipeRead> {
    let mut chunk = [0_u8; 4096];
    loop {
        if let Some(newline) = encoded.iter().position(|byte| *byte == b'\n') {
            return Ok(ReadinessPipeRead::Frame(newline));
        }
        // SAFETY: fd is borrowed from the live readiness-pipe owner, and
        // chunk is a live writable buffer for exactly this syscall.
        ...
        if bytes == 0 {
            return Ok(ReadinessPipeRead::Eof);
        }
        let error = std::io::Error::last_os_error();
        match error.raw_os_error() {
            Some(libc::EINTR) => continue,
            Some(libc::EAGAIN) => return Ok(ReadinessPipeRead::Pending),
...
#[cfg(any(unix, test))]
fn decode_readiness_line(encoded: &[u8]) -> Result<WorkerReadinessLine> {
    serde_json::from_slice(encoded).context("decoding worker readiness hello")
}
...
fn wait_ready_report(
    ...
    timeout: Duration,
) -> Result<Option<WorkerReadinessLine>> {
    use std::os::fd::AsRawFd as _;
    ...
    loop {
        // Observe exit before draining: everything a worker wrote before it
        // exited (notably its boot-failure report) is then already in the
        // pipe, so a fast-failing worker's report is never lost to the race.
        let exited = child.try_wait()?.is_some();
        match drain_readiness_pipe(fd, &mut encoded)? {
            ReadinessPipeRead::Frame(newline) => {
                return decode_readiness_line(&encoded[..newline]).map(Some);
            }
            ReadinessPipeRead::Eof if encoded.is_empty() => return Ok(None),
            ReadinessPipeRead::Eof => return decode_readiness_line(&encoded).map(Some),
            ReadinessPipeRead::Pending if exited => return Ok(None),
            ReadinessPipeRead::Pending => {}
        }
        ...
        if ready < 0 {
            ...
            if error.raw_os_error() != Some(libc::EINTR) {
                return Err(error).context("polling worker readiness");
            }
        }
    }
}
...
fn enforce_unix_worker_readiness(
    ...
    match wait_ready_report(read, child, timeout) {
        Ok(Some(WorkerReadinessLine::BootFailed(failure)))
            if failure.protocol_version == super::proto::PROTOCOL_VERSION
                && failure.pid == child.id()
                && failure.generation == expected_generation =>
        {
            // The worker's own fatal boot error: carry it, typed, instead of
            // the bare "exited before signaling readiness".
            super::spawn_notify::reap_or_kill(child);
            Err(anyhow::Error::new(WorkerBootFailed {
                pid: failure.pid,
                generation: failure.generation,
                reason: failure.boot_failure,
            }))
        }
        Ok(Some(WorkerReadinessLine::Ready(report)))
...
async fn release_ready_successor(
    ...
    #[cfg(unix)]
    {
        ...
        await_successor_ready_construction(worker).await?;
    }
    #[cfg(not(unix))]
...
/// How long the successor's hello may stay unreadable before the roll stops
/// waiting on it: it already reported serving, so a probe problem must not
/// hold the roll for the whole construction budget.
#[cfg(unix)]
const SUCCESSOR_PHASE_PROBE_GRACE: Duration = Duration::from_secs(10);
#[cfg(unix)]
const SUCCESSOR_PHASE_POLL: Duration = Duration::from_millis(50);

/// A roll replaces a worker serving ready services. A successor whose vault
/// is committed serves the locked bootstrap surface first and builds its
/// ready services in the background (the hello's ready-construction phase).
/// The roll is complete only once that construction settles: otherwise the
/// supervisor would report (and clients would reconnect to) an owner that
/// answers every client with a locked hello, which its predecessor never
/// did. A successor awaiting onboarding, or whose construction failed into
/// the retryable phase, has settled; its hello says so. Bounded by the
/// construction's own deadline; the successor exiting meanwhile fails the
/// release, so the caller's recovery path takes over.
#[cfg(unix)]
async fn await_successor_ready_construction(worker: &mut Worker) -> Result<()> {
    let Some(socket) = worker.control_socket.clone() else {
        return Ok(());
    };
    let deadline = tokio::time::Instant::now()
        + super::server::READY_CONSTRUCTION_TIMEOUT
        + Duration::from_secs(30);
    let mut unreadable_since: Option<tokio::time::Instant> = None;
    loop {
        if let Some(child) = worker.child.as_mut()
            && let Some(status) = child
                .try_wait()
                .context("polling released successor during ready construction")?
        {
            bail!(
                "released successor pid {} exited ({status}) while constructing ready services",
                worker.pid
            );
        }
        let now = tokio::time::Instant::now();
        ...
        .await
        {
            Ok(Ok(Some(cockpit_proto::LockedReadyConstruction::Constructing))) => {
                unreadable_since = None;
            }
            Ok(Ok(_)) => return Ok(()),
            Ok(Err(_)) | Err(_) => {
                let since = *unreadable_since.get_or_insert(now);
                if now.duration_since(since) >= SUCCESSOR_PHASE_PROBE_GRACE {
                    tracing::warn!(
                        pid = worker.pid,
                        "released successor hello stayed unreadable; not waiting for its ready construction"
                    );
                    return Ok(());
                }
            }
        ...
        if now >= deadline {
            bail!(
                "released successor pid {} did not settle its ready construction within {}s",
                worker.pid,
                (super::server::READY_CONSTRUCTION_TIMEOUT + Duration::from_secs(30)).as_secs()
            );
        }
        tokio::time::sleep(SUCCESSOR_PHASE_POLL).await;
    }
}
...
mod tests {
    ...
    /// A worker that fails before readiness reports its own error chain on
    /// the readiness pipe and exits at once. The supervisor's error is then
    /// that typed failure (not "exited before signaling readiness"), even
    /// when the exit is observed before the frame is read.
    #[cfg(unix)]
    #[test]
    fn worker_boot_failure_report_reaches_the_supervisor_error() {
        use std::io::Write as _;

        let directory = tempfile::tempdir().unwrap();
        let log_path = directory.path().join("daemon.log");
        std::fs::write(&log_path, b"worker-log-line\n").unwrap();
        let (read, write) = create_ready_pipe().unwrap();
        let mut child = std::process::Command::new("sh")
            .args(["-c", "exit 1"])
            .spawn()
        ...
        serde_json::to_writer(
            &mut writer,
            &WorkerReadinessLine::BootFailed(WorkerBootFailureReport {
                protocol_version: super::super::proto::PROTOCOL_VERSION,
                pid: child.id(),
                generation: 3,
                boot_failure: "opening database: incompatible prerelease database schema v2"
                    .to_string(),
            }),
        )
        .unwrap();
        ...
        drop(writer);
        // The worker is already gone before the supervisor looks.
        child.wait().unwrap();
        ...
        let failure = error
            .downcast_ref::<WorkerBootFailed>()
            .expect("typed worker boot failure");
        assert_eq!(failure.pid, child.id());
        assert_eq!(failure.generation, 3);
        let message = format!("{error:#}");
        assert!(
            message.contains("incompatible prerelease database schema v2"),
            "{message}"
        );
        // Survives the daemon-log rendering a supervisor prints its error with.
        assert!(
            super::super::spawn_notify::error_without_log_tail(&error)
                .contains("incompatible prerelease database schema v2")
        );
    }

    /// A ready frame keeps its exact wire shape and never decodes as a boot
    /// failure, and vice versa.
    #[test]
    fn readiness_frames_decode_by_shape() {
        let ready = serde_json::to_vec(&WorkerReadyReport {
            protocol_version: 1,
            pid: 2,
            generation: 3,
            opened_at_unix_ms: 4,
        ...
        .unwrap();
        assert!(matches!(
            decode_readiness_line(&ready).unwrap(),
            WorkerReadinessLine::Ready(_)
        ));
        let failed =
            serde_json::to_vec(&WorkerReadinessLine::BootFailed(WorkerBootFailureReport {
                protocol_version: 1,
                pid: 2,
                generation: 3,
                boot_failure: "boom".to_string(),
            }))
            .unwrap();
        assert!(!String::from_utf8_lossy(&failed).contains("opened_at_unix_ms"));
        assert!(matches!(
            decode_readiness_line(&failed).unwrap(),
            WorkerReadinessLine::BootFailed(report) if report.boot_failure == "boom"
        ));
    }
