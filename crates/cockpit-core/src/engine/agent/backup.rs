...
pub async fn turn_with_backup(
    ...
    // Per-round-trip id generated by the driver and shared with the primary
    // tandem records. Backup attempts allocate their own audit identity so a
    // terminal primary record cannot absorb the backup's request body.
    call_id: Uuid,
    ...
    loop {
        ...
        let attempt_call_id = if attempt_index == 0 {
            call_id
        } else {
            Uuid::new_v4()
        };
...
pub(super) async fn record_inference_outcome(ctx: InferenceOutcomeRecord<'_>, err: &anyhow::Error) {
    ...
    if record_inference_request_async(session.clone(), call_id, terminal, status, goal_provenance)
        .await
        .is_err()
    {
