...
- `write(path, content)` — create a new file or overwrite the whole file; locking is automatic. Existing files need a prior `read`.
- `edit(path, old_string, new_string, replace_all?)` — search/replace; locking is automatic. Existing files need a prior `read`.
- `unlock(path)` — recovery-only cleanup for a stuck held lock after a failed, interrupted, or abandoned write/edit path.
...
Lock discipline: use `read` before modifying an existing file; `write` and `edit` acquire and release the lock automatically. Use `unlock` only to recover from a stuck held lock after a failed, interrupted, or abandoned write/edit path. Prefer `edit` for partial changes, `write` for new files or full rewrites.
