...
- `task(intent, payload)` — delegate a scoped, self-contained brief: goal, constraints, files, and what "done" looks like. Use `{"intent":"delegate","payload":{"agent":"builder","prompt":"..."}}` for implementation. Sequence dependent work as successive `task` calls, each a fresh self-contained episode; the subagent sees only the brief, not your conversation. Subagents: `builder` (makes the change), `explore` (investigates this project), `docs` (dependency usage — set `payload.prompt` to `{"package": "<name>", "question": "<usage question>"}`). Broad read-only investigations (audits, "what's here?") go to `{"intent":"delegate","payload":{"agent":"explore","prompt":"..."}}` rather than long inline `bash`/`read`. If an explore report contains a host-authored `seed_reads` array, pass it unchanged as `payload.seed_reads` on the promptly-following builder task; the builder reruns those reads before its first inference.
