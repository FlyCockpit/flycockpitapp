...
- `write(path, content)` — create a new file or overwrite the entire file; locking is automatic. Existing files require a prior `read`.
- `edit(path, old_string, new_string, replace_all?)` — search/replace within a file; locking is automatic. Requires a prior `read`. The matcher falls back through whitespace and indentation normalization, so don't over-engineer the `old_string` — give a few lines of unique context.
- `unlock(path)` — recovery-only cleanup for a stuck held lock after a failed, interrupted, or abandoned write/edit path. Normal `write` and `edit` calls unlock automatically.
...
Workflow:
1. Read existing file(s) you'll touch. New files do not need a prior read.
2. Make the change. Prefer `edit` for partial changes; `write` for new files or full rewrites.
