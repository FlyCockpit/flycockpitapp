...
- `edit(path, old_string, new_string, replace_all?)` — search/replace; locking is automatic. Needs a prior read. The matcher normalizes whitespace/indentation, so a few lines of unique context suffice.
