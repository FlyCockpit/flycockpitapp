...
model using that worker's exact `model` and an empty `write_scope`, run selected harnesses through the MCP
harness advert with isolated review-only policy, await reports, spawn focused `scout` tiebreakers
(also empty `write_scope`) for genuine conflicts, and return one consolidated severity-ranked analysis
