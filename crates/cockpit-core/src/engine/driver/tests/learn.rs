...
#[test]
fn learn_saves_conformant_foreground_skill() {
...
#[test]
fn learn_respects_write_gate() {
    crate::test_env::run_async_with_large_stack(|| async {
        let (mut driver, tmp, root, provider) = learn_driver(true, "gated-learn", 1);
        let policy = crate::config::trust::WorkspaceTrustPolicy {
            root: crate::config::trust::resolve_trust_root(tmp.path()).unwrap(),
            mode: crate::db::workspace_trust::WorkspaceTrustMode::Trust,
        };
        ...
        let task = tokio::spawn(async move {
            crate::config::trust::scope_workspace_trust_policy(
                ...
                driver.run_user_input(
                    UserSubmission::text(crate::skills::build_learn_prompt(
