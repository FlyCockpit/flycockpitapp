//! OpenRouter Images runtime health/capability probe adapter.
//!
//! Implements adapter kind [`ImageAdapterKind::OpenrouterImages`] as a
//! read-only health/capability probe against the configured OpenRouter origin.
//!
...
//! ephemeral credential/attribution headers that are never forwarded across a
//! redirect boundary. This adapter never constructs an HTTP client and never
...
/// The OpenRouter Images runtime health/capability probe adapter.
///
/// The credential boundary (the `Authorization: Bearer` header) and OpenRouter
/// attribution headers are resolved by the registry into the ephemeral header
/// map and are never forwarded across a redirect boundary (enforced by the
/// registry's connector).
pub struct OpenrouterImagesRuntimeAdapter {
    kind: ImageAdapterKind,
}

impl OpenrouterImagesRuntimeAdapter {
    pub fn new() -> Self {
        Self {
            kind: ImageAdapterKind::OpenrouterImages,
        }
    }
}

impl Default for OpenrouterImagesRuntimeAdapter {
    fn default() -> Self {
        Self::new()
    }
}

impl adapter_sealed::Sealed for OpenrouterImagesRuntimeAdapter {}

impl super::ImageRuntimeAdapter for OpenrouterImagesRuntimeAdapter {
    ...
    fn request(&self, request: &ProbeRequest) -> Result<ReadOnlyProbeRequest, RuntimeError> {
        ...
        let route_url = request
            ...
            .map_err(|_| {
                RuntimeError::new(
                    ...
                    "Correct the OpenRouter Images endpoint origin.",
                )
            })?;
        ...
        // The registry has already resolved the credential/attribution headers
        // into the ephemeral header map. Credentials are never forwarded across
        // redirects.
...
/// Build the production standard adapter set entry for OpenRouter images.
pub fn standard_adapter() -> Arc<dyn super::ImageRuntimeAdapter> {
    Arc::new(OpenrouterImagesRuntimeAdapter::new())
}
