...
impl HostContext {
    ...
    #[cfg(test)]
    pub fn with_tool_steering_for_tests(mut self, steering: crate::agents::ToolSteering) -> Self {
        self.tool_steering = steering;
        self
    }
...
pub fn presentation(tool: &str, args: &Value) -> Option<ToolPresentation> {
    ...
    let display_args = display_args.clone();
...
pub fn search(ctx: &HostContext, query: &str) -> Vec<SearchHit> {
    ...
    let steering = ctx.tool_steering;
...
mod tests {
    ...
    /// The agent-facing sealed-value lifecycle builtins are gone.
    ///
    /// They let any untrusted Monty agent create and overwrite a session
    /// sealed value with no Owner authority, no record, and no grant — an
    /// agent-controlled lifecycle the spec forbids outright. Worse, they
    ...
    fn monty_exposes_no_sealed_lifecycle_or_existence_oracle() {
        ...
        for retired in ["set_sealed_value", "request_sealed_value"] {
            assert!(
                registry.get(retired).is_none(),
                "`{retired}` must not be reachable from Monty"
            );
            assert!(
                presentation(retired, &serde_json::json!({})).is_none(),
                "`{retired}` must not retain a presentation entry"
            );
        }
        ...
        keys.sort();
        let mut expected = crate::sealed::USE_SEALED_VALUE_ARG_KEYS.to_vec();
        expected.sort();
        assert_eq!(keys, expected);

        // No source-level status branch survives that could distinguish a
        // created value from an overwritten one.
        ...
        assert!(
            !production.contains("\"overwritten\"") && !production.contains("store_sealed_value"),
            "no sealed create/overwrite status branch may remain"
        );
    }
    ...
    fn host_with_tool(root: &std::path::Path, tool: Arc<dyn Tool>) -> HostContext {
        host_with_tool_mode(root, tool, crate::agents::ToolSteering::Terse)
    }
    ...
    fn monty_adapter_preserves_schema_structure_for_every_builtin() {
            ...
            .filter(|tool| crate::engine::tool::is_monty_builtin_adaptable(tool.name()))
        {
            ...
            let normal_descriptor = func.descriptor(crate::agents::ToolSteering::Terse);
            assert_faithful_schema(
                &normal_schema,
                &normal_descriptor.input_schema,
                &format!("{} Terse schema", tool.name()),
            );

            let defensive_schema = tool
                .verbose_parameters()
                .unwrap_or_else(|| tool.parameters());
            let defensive_descriptor = func.descriptor(crate::agents::ToolSteering::Verbose);
            assert_faithful_schema(
                &defensive_schema,
                &defensive_descriptor.input_schema,
                &format!("{} Verbose schema", tool.name()),
            );
    ...
    fn monty_direct_call_marker_present_in_both_modes() {
        ...
        .into_function()
        .unwrap();
        let normal = func.descriptor(crate::agents::ToolSteering::Terse);
        let defensive = func.descriptor(crate::agents::ToolSteering::Verbose);
    ...
    fn monty_search_matches_defensive_text_in_defensive_mode() {
        ...
        let defensive_host = host_with_tool_mode(
            tmp.path(),
            tool.clone(),
            crate::agents::ToolSteering::Verbose,
        );
    ...
    fn write_trusted_openai_provider(root: &std::path::Path, trust: &str) {
        ...
        std::fs::write(root.join(".cockpit").join("config.json"), "{}").unwrap();
        std::fs::write(
            providers.join("openai.json"),
            serde_json::json!({
                "url": "https://example.test/v1",
                "models": [{"id": "gpt-5", "trust": trust}],
            })
            .to_string(),
        )
        .unwrap();
