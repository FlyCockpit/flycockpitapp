...
impl super::Db {
    ...
    pub fn discard_unreferenced_media_attachment_conn(
        ...
        for (component_id, kind, generation) in &components {
            ...
            hasher.update([0]);
            // The component-set digest keeps the decimal-text field encoding
...
mod tests {
    ...
    fn component_lease_authority_columns_live_only_on_lease_table_and_reopen() {
        ...
        let lease_id = Uuid::now_v7();
        {
            ...
            db.blocking_for_sync_cli(move|conn|{
                ...
                conn.execute("INSERT INTO sessions(session_id,project_id,project_root,started_at_unix_ms,last_active_at_unix_ms,short_id) VALUES(?1,'p','/project',1,1,lower(hex(randomblob(3))))",[session_id.to_string()])?;
    ...
    async fn media_attachment_transaction_exposes_readiness_ownership_and_capability_generation() {
        ...
        db.transaction(move |conn| {
            conn.execute(
                "INSERT INTO sessions (session_id,project_id,project_root,started_at_unix_ms,last_active_at_unix_ms,short_id) VALUES (?1,'project','/redacted',1,1,lower(hex(randomblob(3))))",
    ...
    async fn ready_component_lease_serializes_with_cleanup_and_capability_rotation() {
        ...
        db.transaction(move |conn| {
            conn.execute("INSERT INTO sessions(session_id,project_id,project_root,started_at_unix_ms,last_active_at_unix_ms,short_id) VALUES(?1,'p','/redacted',1,1,lower(hex(randomblob(3))))",[session_id.to_string()])?;
