...
CREATE TRIGGER media_reservation_version_insert AFTER INSERT ON media_reservations
BEGIN
    INSERT INTO media_reservation_versions(reservation_id,version,state,recorded_wall_ms)
    VALUES(NEW.reservation_id,NEW.version,NEW.state,NEW.created_wall_ms);
END;
CREATE TRIGGER media_reservation_version_update AFTER UPDATE OF version ON media_reservations
BEGIN
    INSERT INTO media_reservation_versions(reservation_id,version,state,recorded_wall_ms)
    VALUES(NEW.reservation_id,NEW.version,NEW.state,CAST(strftime('%s','now') AS INTEGER)*1000);
END;
CREATE TABLE media_queue_sequence (
    singleton INTEGER PRIMARY KEY CHECK(singleton=1),
    next_value INTEGER NOT NULL CHECK(next_value >= 1)
);
INSERT INTO media_queue_sequence(singleton,next_value) VALUES(1,1);
CREATE TABLE media_scheduler_cursor (
    singleton INTEGER PRIMARY KEY CHECK(singleton=1),
...
CREATE TABLE media_artifact_facts (
    ...
    deletion_tombstone_checksum TEXT
);
...
CREATE TABLE media_repair_attempts (
    ...
    outcome TEXT,
    current_counter_digest TEXT NOT NULL,
    rebuilt_counter_digest TEXT,
    created_wall_ms INTEGER NOT NULL,
...
CREATE TRIGGER media_repair_state_graph BEFORE UPDATE OF state ON media_repair_attempts
WHEN NOT (
    (OLD.state='planned' AND NEW.state='rebuilding') OR
    (OLD.state='rebuilding' AND NEW.state='verifying') OR
    (OLD.state='verifying' AND NEW.state IN ('committed','failed')) OR
    OLD.state=NEW.state
)
BEGIN
    SELECT RAISE(ABORT, 'invalid media repair state transition');
END;
