...
CREATE TABLE sessions (
    ...
    short_id           TEXT NOT NULL CHECK (
        length(short_id) = 6
        AND short_id = lower(short_id)
        AND short_id NOT GLOB '*[^0123456789abcdefghjkmnpqrstvwxyz]*'
    ),                     -- 6-char Crockford base32 display id, generated at every insert
...
-- A short_id is unique within its project. Session inserts rely on this
-- constraint to detect a lost generation race and retry with a fresh id
-- (src/db/sessions.rs).
...
-- ============================================================================
-- ==== extended domains (scheduler, image spend, image generation) ===========
-- ============================================================================
--
...
-- Leading child indexes for extended-profile foreign keys.
CREATE INDEX idx_image_generation_response_publication_intents_attempt
 ON image_generation_response_publication_intents(job_id, slot_id, attempt_number);
CREATE INDEX idx_image_generation_attempt_media_snapshots_plan
 ON image_generation_attempt_media_snapshots(job_id, plan_digest);
CREATE INDEX idx_image_generation_deadline_expiry_facts_cancellation
 ON image_generation_deadline_expiry_facts(job_id, cancellation_version);
CREATE INDEX idx_image_generation_cancelled_result_facts_cancellation
 ON image_generation_cancelled_result_facts(job_id, cancellation_version);
CREATE INDEX idx_image_generation_publication_right_facts_attempt
 ON image_generation_publication_right_facts(job_id, slot_id, attempt_number);
CREATE INDEX idx_image_generation_artifact_references_artifact
 ON image_generation_artifact_references(artifact_id);
CREATE INDEX idx_image_generation_artifact_authorization_facts_artifact
 ON image_generation_artifact_authorization_facts(artifact_id);
CREATE INDEX idx_image_generation_artifact_authorization_facts_slot
 ON image_generation_artifact_authorization_facts(job_id, slot_id);
CREATE INDEX idx_image_generation_artifact_leases_component
 ON image_generation_artifact_leases(artifact_id, component_id);
CREATE INDEX idx_image_generation_artifact_leases_owning_slot
 ON image_generation_artifact_leases(owning_job_id, owning_slot_id);
CREATE INDEX idx_image_generation_late_publication_leases_artifact
 ON image_generation_late_publication_leases(artifact_id);
CREATE INDEX idx_image_generation_late_publication_leases_slot
 ON image_generation_late_publication_leases(job_id, slot_id);
CREATE INDEX idx_image_generation_late_publication_authorization_facts_artifact
 ON image_generation_late_publication_authorization_facts(artifact_id);
CREATE INDEX idx_image_generation_late_publication_authorization_facts_slot
 ON image_generation_late_publication_authorization_facts(job_id, slot_id);
CREATE INDEX idx_image_generation_user_published_outputs_artifact
 ON image_generation_user_published_outputs(artifact_id);
CREATE INDEX idx_image_generation_artifact_security_recovery_audits_artifact
 ON image_generation_artifact_security_recovery_audits(artifact_id);
CREATE INDEX idx_image_generation_artifact_security_recovery_audits_slot
...
-- ============================================================================
-- ==== remote domains (org sync, connectors, remote attachments, custody) ====
-- ============================================================================
--
-- Created unconditionally in every build. Only code compiled with the
