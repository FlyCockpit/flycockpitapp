...
impl Db {
    ...
    ///
    /// Session literals never leave SQLite, so the record row and the literal
    /// are written in **one transaction** and there is no cross-store saga to
    /// interrupt: the value is either fully absent or fully live.
    ...
    /// is not.
    ///
    /// Returns `None` for a non-resolvable record and for a version mismatch,
    /// so a partially staged or rotated value can never be read.
    pub async fn sealed_session_literal_for_action(
        &self,
        record_id: String,
        claimed_version: i64,
        ...
        self.read(move |conn| {
            ...
            .optional()
            .context("resolving session sealed literal")
        })
        .await
    }
