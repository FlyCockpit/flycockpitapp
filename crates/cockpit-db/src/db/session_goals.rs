...
fn finish_control_row(
    ...
    let stored = match result {
        Some(raw) if serde_json::from_str::<serde_json::Value>(raw).is_ok() => Some(raw.to_owned()),
        Some(raw) => {
            Some(serde_json::to_string(raw).context("encoding non-JSON control job result")?)
        }
        None => None,
    };
    conn.execute(
