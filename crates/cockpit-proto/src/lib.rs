...
/// Current wire schema version. v22 adds rolling-precompaction resume choice:
/// `resume_from_compaction` is a remotely dispatchable request and `attached`
/// can carry the exact-snapshot offer. v21 and every older fixture remain
/// frozen migration evidence, not a compatibility window.
pub const PROTOCOL_VERSION: u32 = 22;

/// Oldest wire schema version this binary accepts. Exact-match only until a
/// compacted v1 ships. v22 is current-only: the rolling-precompaction resume
/// contract is an explicit breaking change with no compatibility shim.
