...
pub use acp::{
    ...
    CreateCodeRootWithAcpIngressV1Request, CreateCodeRootWithAcpIngressV1Result,
...
/// Authorship namespace shown only on the owner inventory surface.
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum SealedOwnerNamespace {
    OwnerAuthored,
    AgentAcquired,
}
...
pub struct SealedOwnerInventoryItem {
    ...
    pub namespace: SealedOwnerNamespace,
}

/// Secret-free owner audit row for one trusted-child acquisition attempt.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct SealedAcquisitionAuditItem {
    ...
    fn supported_fixture_directories() -> BTreeSet<u32> {
        ...
        for entry in entries {
            ...
            if name == "archive.sha256" {
                assert!(path.is_file(), "archive checksum manifest must be a file");
                continue;
            }
            assert!(
                path.is_dir(),
