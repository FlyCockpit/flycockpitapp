...
use crate::tui::agent_runner::{
    AgentRunner, ControlRequest, RunnerInput, SessionSwitchOutcome, SessionTarget,
    TestRunnerOverrides,
};
...
fn async_dispatch_failure_reconciles_exact_row_before_next_record_succeeds() {
    ...
    app.apply_event(
        cockpit_client::presentation::TurnEvent::UserMessageDispatchFailed {
            error: "image upload rejected".to_string(),
            optimistic_submission_id: first.optimistic_submission_id,
        },
    );
    app.apply_event(
        cockpit_client::presentation::TurnEvent::UserMessageRecorded {
            seq: 91,
...
fn fresh_a_failure_then_busy_b_fold_reconciles_history_queue_and_duplicate_record() {
    ...
    app.apply_event(
        cockpit_client::presentation::TurnEvent::QueuedUserMessagesFolded {
