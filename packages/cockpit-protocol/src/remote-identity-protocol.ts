...
export function encodePossessionProof(v: PossessionProofV1) {
  ...
  exact(v.signatureP1363, 64, "signature");
...
export function parseRemoteIdentityCertificateJws(compact: string) {
  ...
  const decode = (s: string) => {
    if (!/^[A-Za-z0-9_-]+$/.test(s)) fail("noncanonical base64url");
    const padded = s.replace(/-/g, "+").replace(/_/g, "/") + "===".slice((s.length + 3) % 4);
    const bytes = Uint8Array.from(atob(padded), (c) => c.charCodeAt(0));
    return bytes;
  };
  const header = JSON.parse(td.decode(decode(parts[0]!))) as unknown;
  const payload = JSON.parse(td.decode(decode(parts[1]!))) as unknown;
  const sig = decode(parts[2]!);
  exact(sig, 64, "JWS signature");
