...
describe("send_user_message_v2_canonical_vectors", () => {
  it("round trips the shared bytes and digests", async () => {
    ...
    expect(toHex(encodeCanonicalSendUserMessageV2(decoded))).toBe(vector.fcm2_hex);
    expect(toHex(await messageRequestDigest(decoded))).toBe(vector.message_request_digest_hex);
    expect(toHex(await attachmentSetDigest(decoded))).toBe(vector.attachment_set_digest_hex);
  });
