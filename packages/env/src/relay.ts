...
export const env = createEnv({
  server: {
    ...
    RELAY_MODE: z.enum(["embedded", "shared-secret", "fleet"]).default("embedded"),
    RELAY_BIND_ADDR: z.ipv4().or(z.ipv6()).optional(),
    RELAY_CERTIFICATE_PATH: z.string().trim().min(1).optional(),
    RELAY_PRIVATE_KEY_PATH: z.string().trim().min(1).optional(),
