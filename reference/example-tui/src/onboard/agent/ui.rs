//! Shared chrome for the agent sub-wizard: the warm instrument-panel palette
//! (kept in step with the earlier screens on purpose), the header/help/column
//! helpers, a bordered single-line field, a hand-drawn scrollbar, and a small
//! [`ListNav`] cursor so every list screen scrolls and pages the same way.
...
pub(super) const INK: Color = Color::Rgb(0xF4, 0xEF, 0xE6);
pub(super) const FOG: Color = Color::Rgb(0x8A, 0x9B, 0xB0);
pub(super) const BRASS: Color = Color::Rgb(0xE0, 0xB1, 0x56);
pub(super) const NIGHT: Color = Color::Rgb(0x4A, 0x5A, 0x6A);
pub(super) const GOOD: Color = Color::Rgb(0x7F, 0xC9, 0x8A);
pub(super) const WARN: Color = Color::Rgb(0xD8, 0x8A, 0x5A);
pub(super) const DISABLED: Color = Color::Rgb(0x55, 0x62, 0x70);
pub(super) const HOVER_BG: Color = Color::Rgb(0x2C, 0x38, 0x46);
pub(super) const PLACEHOLDER: Color = Color::Rgb(0x6A, 0x7A, 0x8A);
...
/// Mirrors [`super::super::verify`]'s bar so overflow reads identically.
